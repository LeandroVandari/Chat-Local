...
impl Client {
    pub fn new() -> Self {
        ...
        let listener = TcpListener::bind((std::net::Ipv4Addr::UNSPECIFIED, 0)).expect("Couldn't create listener");
        let conn_request = super::ConnectionRequest::new(&listener);
        let request_json = serde_json::to_string(&conn_request).unwrap();

        udp_sock.send_to(request_json.as_bytes(), &addrs::SOCKET_ADDR).expect("Couldn't send connection request to server");
